...
INDEX_SYMBOL = ['^DJI']
STOCK_SYMBOLS = ['AXP','AMGN','AAPL','BA','CAT','CSCO','CVX','GS','HD','HON','IBM','INTC','JNJ','KO','JPM','MCD','MMM','MRK','MSFT','NKE','PG','TRV','UNH','CRM','VZ','V','WBA','WMT','DIS','DOW']
#**************** FUNCTIONS TO FETCH DATA ***************************
def get_historical(symbol):
    ticker = symbol
    stock_info = yf.download(symbol)
    myf=str(symbol) + str(".csv")
    ...
    if request.method == 'POST':
        ...
        if request.form['submit'] == 'calculate_trade':
            return render_template('index.html',
                ...
                short_size = round((float(selected_budget) ) / short_last_close,2),
                long_last_close = round(long_last_close,2),
                long_size = round((float(selected_budget) ) / long_last_close,2),
